...
- `secretaria`: pertenece a un `admin` (`admin_id` en `users`). Su alcance real es acotado: **ve, crea y corrige citas** de ese médico — puede editar `fecha`/`hora`/`motivo` para arreglar errores de carga, pero no marcarlas como atendidas ni eliminarlas (`CitaPolicy::update()` ya la incluye, pero `CitaController::update()` le ignora el campo `estado` aunque lo mande, y `CitaPolicy::delete()` la sigue excluyendo), y **no** accede a Pacientes como pantalla propia ni a historial médico/recetas. La única forma en que toca datos de Pacientes es de forma incidental, dentro del propio flujo de "Nueva cita" (buscar uno existente o crear uno nuevo ahí mismo sin pasar por la gestión de Pacientes) — por eso `PatientPolicy::create()`/`viewAny()` siguen permitiéndole crear/listar pacientes, pero `update()` no.
