...
- `GET/PUT /api/v1/users/{user}/branding`, `POST /api/v1/users/{user}/branding/logo`, `POST /api/v1/users/{user}/branding/header-icon-left` y `.../header-icon-right` — **solo el Superadmin** (middleware `role:superadmin`), gestionando la marca de un médico puntual por su id desde la pantalla de Usuarios. El propio médico (`admin`) no puede editar su marca.
